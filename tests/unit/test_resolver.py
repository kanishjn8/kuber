...
def test_cluster_role_binding_is_cluster_wide() -> None:
    policy = resolve_effective_policy(parse_rbac(_manifest("ClusterRoleBinding")), ServiceAccountRef("app", "team-a"))
...
def test_role_binding_scopes_cluster_role_to_namespace() -> None:
    policy = resolve_effective_policy(parse_rbac(_manifest("RoleBinding")), ServiceAccountRef("app", "team-a"))
    assert {item.namespace for item in policy.permissions} == {"team-a"}


def test_unbound_service_account_gets_no_permissions() -> None:
    policy = resolve_effective_policy(parse_rbac(_manifest("ClusterRoleBinding")), ServiceAccountRef("other", "team-a"))
