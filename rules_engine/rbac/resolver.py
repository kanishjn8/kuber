    ...
    for binding in parsed.bindings:
        ...
        role = next(
            (
                ...
                and (
                    (binding.role_kind == "ClusterRole" and candidate.cluster_role)
                    or (
                        binding.role_kind == "Role"
                        and not candidate.cluster_role
                        and candidate.namespace == binding.namespace
                    )
                )
            ),
            None,
        ...
        source = (
            f"{'ClusterRoleBinding' if binding.cluster_binding else 'RoleBinding'}/{binding.name}"
        )
                ...
                for resource in rule.resources:
                    for verb in rule.verbs:
                        for resource_name in names:
                            permissions.append(
                                Permission(group, resource, verb, namespace, resource_name, source)
                            )
    return Policy(
