...
T = TypeVar("T")
LOGGER = logging.getLogger("kuber.events")
...
class TrackedKubeClient:
    ...
    def _event(self, api_group: str, resource: str, verb: str, resource_name: str | None = None) -> dict[str, Any]:
        return {
            "kuber_event": True,
            "api_group": api_group,
            "resource": resource,
    ...
    def _call(self, event: dict[str, Any], operation: Callable[[], T]) -> T:
        ...
        try:
            ...
            if exc.status == 403:
                LOGGER.error(json.dumps({**event, "kuber_denied": True, "message": exc.reason}, sort_keys=True))
            raise

    def read_config_map(self, name: str) -> dict[str, str]:
        event = self._event("", "configmaps", "get", name)
        value = self._call(event, lambda: self.core.read_namespaced_config_map(name, self.namespace))
            ...
            spec=client.V1JobSpec(
                template=client.V1PodTemplateSpec(
                    ...
                    spec=client.V1PodSpec(
                        ...
                        containers=[client.V1Container(name="reconcile", image="busybox:1.36", command=["sh", "-c", "echo reconciled"])],
                    ),
                ),
                backoff_limit=0,
            ),
    ...
    def delete_job(self, name: str) -> None:
        ...
        self._call(event, lambda: self.batch.delete_namespaced_job(name, self.namespace, propagation_policy="Background"))
    ...
    def renew_leader_lease(self, name: str) -> str:
        ...
        try:
            lease = self._call(read_event, lambda: self.coordination.read_namespaced_lease(name, self.namespace))
            update_event = self._event("coordination.k8s.io", "leases", "update", name)
            self._call(update_event, lambda: self.coordination.replace_namespaced_lease(name, self.namespace, lease))
            return "updated"
        except ApiException as exc:
            if exc.status != 404:
                raise
        create_event = self._event("coordination.k8s.io", "leases", "create", name)
        body = client.V1Lease(metadata=client.V1ObjectMeta(name=name), spec=client.V1LeaseSpec(holder_identity="payment-controller"))
        self._call(create_event, lambda: self.coordination.create_namespaced_lease(self.namespace, body))
        return "created"
